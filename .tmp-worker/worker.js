...
function getRepoRoot() {
  ...
  if (override) return override;
  return join2(dirname(fileURLToPath2(import.meta.url)), "..", "..", "..", "..");
}
...
async function composeDownBestEffort(runner, ctx) {
  const result = await runner.run(ctx.composeFile, ctx.project, ctx.envPath, ctx.composeEnv, ["down"]).then(() => true).catch(() => false);
  return result;
}
...
var FetchStockixFinanceBootstrap = class {
  ...
  async registerBootstrapAdmin(params) {
    ...
    throw new Error(`register failed: HTTP ${res.status} ${text2.slice(0, 500)}`);
  }
};
...
async function runTenantLifecycleCommand(db, job, command) {
  ...
  await execa2("docker", ["compose", "-p", row.composeProjectName, command], {
    timeout: 6e4
  });
}
var handlers = {
  ...
  while (!shuttingDown) {
    ...
    try {
      ...
      try {
        ...
        await markJobFailure(job.id, message, cancelledByUser);
