...
Root scripts orchestrate Postgres + Redis (`db:up`, `db:migrate`), `@repo/auth` build (CJS for POS), worker build/run, and `concurrently` for API + dashboard + worker + **POS stack** (`scripts/dev-pos-stack.mjs`: API **8010**, restaurant UI **3001**). Skip POS with `STOCKIX_DEV_SKIP_POS=1 pnpm dev`. POS packages are installed with root `pnpm install` (see `pnpm-workspace.yaml`).
