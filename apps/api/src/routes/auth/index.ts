...
export function buildAuthRoutes(db: PostgresJsDatabase<typeof schema>) {
  ...
  auth.get("/me", async (c) => {
    ...
    // Resolve custom role / permissions from the DB; fall back to built-in role
    // permissions when the DB mock used in unit tests does not support select.
    ...
    try {
      ...
      if (auth) {
        ...
        if (auth.permissions.length > 0) permissions = auth.permissions;
      }
    } catch {
