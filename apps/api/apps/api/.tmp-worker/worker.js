  ...
  "../../node_modules/.pnpm/dotenv@16.6.1/node_modules/dotenv/lib/main.js"(exports, module) {
    ...
    function configDotenv(options) {
      ...
      for (const path8 of optionPaths) {
        try {
          const parsed = DotenvModule.parse(fs2.readFileSync(path8, { encoding }));
...
// ../../packages/config/src/index.ts
var configDir = path6.dirname(fileURLToPath3(import.meta.url));
var monorepoRoot = path6.join(configDir, "..", "..", "..");
var preferredEnvPath = existsSync(path6.join(monorepoRoot, ".env.local")) ? path6.join(monorepoRoot, ".env.local") : path6.join(monorepoRoot, ".env");
(0, import_dotenv.config)({ path: preferredEnvPath, override: false });
var optionalStringSchema = external_exports.string().min(1).optional();
var stringSchema = external_exports.string().min(1);
var numberSchema = external_exports.coerce.number().finite();
var booleanStringSchema = external_exports.enum(["true", "false", "1", "0"]).optional();
function parseValue(schema, value, name) {
  const result = schema.safeParse(value);
  if (!result.success) {
    throw new Error(`[config] invalid ${name}: ${result.error.issues.map((i2) => i2.message).join(", ")}`);
  }
  return result.data;
}
...
var b = Object.assign(reset2, messages, {
...
function Subscribe(postgres2, options) {
  ...
  async function init(sql3, slot2, publications) {
    ...
    function handle(a2, b2) {
      ...
      call("*", a2, b2);
      call("*:" + path7, a2, b2);
      b2.relation.keys.length && call("*:" + path7 + "=" + b2.relation.keys.map((x2) => a2[x2.name]), a2, b2);
      call(b2.command, a2, b2);
      call(b2.command + ":" + path7, a2, b2);
      b2.relation.keys.length && call(b2.command + ":" + path7 + "=" + b2.relation.keys.map((x2) => a2[x2.name]), a2, b2);
    }
    function pong() {
  ...
  function Sql(handler2) {
    ...
    function file(path7, args = [], options2 = {}) {
      ...
      const query = new Query([], args, (query2) => {
        fs.readFile(path7, "utf8", (err, string) => {
          if (err)
            return query2.reject(err);
...
var PgSelectQueryBuilderBase = class extends TypedQueryBuilder {
  ...
  createJoin(joinType, lateral) {
    return (table, on6) => {
      ...
      this.config.joins.push({ on: on6, table, joinType, alias: tableName, lateral });
...
function getRepoRoot() {
  ...
  if (override) return override;
  const here = dirname(fileURLToPath4(import.meta.url));
    ...
    if (cancelSignal) {
      abortHandler = () => {
        if (process.platform === "win32" && typeof subprocess.pid === "number") {
          void execa("taskkill", ["/PID", String(subprocess.pid), "/T", "/F"]).catch(() => void 0);
          return;
        }
...
import { mkdir as mkdir3, unlink, writeFile as writeFile2 } from "node:fs/promises";
...
async function deprovisionTenant(db, tenantId, options = {}) {
  ...
  const found = await db.select({ id: tenants.id, slug: tenants.slug, composeProject: tenantDeployments.composeProjectName }).from(tenants).leftJoin(tenantDeployments, eq(tenantDeployments.tenantId, tenants.id)).where(eq(tenants.id, tenantId)).limit(1);
  const row = found[0];
...
var provisionPayloadSchema = external_exports.object({
  slug: external_exports.string().min(1),
  name: external_exports.string().min(1),
  ownerId: external_exports.string().uuid(),
  adminEmail: external_exports.string().email(),
  adminFirstName: external_exports.string().min(1),
  adminLastName: external_exports.string().min(1)
});
...
async function runTenantLifecycleCommand(db, job, command) {
  ...
  }).from(tenants).leftJoin(tenantDeployments, eq(tenantDeployments.tenantId, tenants.id)).where(eq(tenants.id, job.tenantId)).limit(1);
  const row = rows[0];
