...
import { Button, Input, ScrollArea, Combobox, ComboboxContent, ComboboxEmpty, ComboboxInput, ComboboxItem, ComboboxList, ComboboxSeparator, ComboboxTrigger, ComboboxValue } from "@repo/ui-core";
...
function normalizeE164Value(value: string | BasePhoneInput.Value | undefined): BasePhoneInput.Value | undefined {
  if (value == null || value === "") return undefined;
  const trimmed = String(value).trim();
  if (!trimmed) return undefined;
  if (/^\+[1-9]\d{1,14}$/.test(trimmed)) return trimmed as BasePhoneInput.Value;
  const digitsOnly = trimmed.replace(/\D/g, "");
  if (!digitsOnly) return undefined;
  const candidate = `+${digitsOnly}`;
  if (/^\+[1-9]\d{1,14}$/.test(candidate)) {
    return candidate as BasePhoneInput.Value;
  }
  return undefined;
}
...
function PhoneInput({
  ...
  const normalizedValue = normalizeE164Value(value);
  return (
...
function InputComponent({ className, ...props }: ComponentProps<typeof Input>) {
  ...
  return (
    <Input
      className={cn("rounded-s-none focus:z-1", variant === "sm" && "h-8", variant === "lg" && "h-10", className)}
      {...props}
    />
  );
}
