          ...
          <ScrollArea className="h-full">
            <div className="flex flex-col gap-1.5 pb-10">
              <button
                ...
                className={cn(
                  "flex h-12 items-center rounded-xl px-5 font-black text-xs uppercase tracking-widest transition-all",
                  selectedCatId === "all"
                ...
                <button
                  ...
                  className={cn(
                    ...
                    locked || it.isAvailable === false
                      ? "cursor-not-allowed border-zinc-900 bg-zinc-950 opacity-40"
                      : "cursor-pointer border-zinc-800 bg-zinc-900/40 shadow-xl hover:border-emerald-500/40 hover:bg-zinc-900/60 active:scale-95",
                  )}
                >
                  <div className="relative z-10 flex flex-col gap-2">
                    <h4 className="font-black text-lg text-white tracking-tight transition-colors group-hover:text-emerald-400">
                      {it.name}
                    </h4>
                    <span className="font-bold text-emerald-500/90 text-sm tabular-nums">
                      {formatCurrency(it.priceUsd || 0, { currency: "USD" })}
                    </span>
                  </div>

                  <div className="absolute -right-12 -bottom-12 h-32 w-32 rounded-full bg-emerald-500/5 blur-3xl transition-all duration-500 group-hover:scale-125 group-hover:bg-emerald-500/10" />

                  {!it.isAvailable && (
                    <div className="absolute top-4 right-4 rounded-full bg-red-500/10 px-2 py-1 font-black text-[9px] text-red-500 uppercase tracking-widest ring-1 ring-red-500/20">
                      Sold Out
                    </div>
                  )}
                </button>
