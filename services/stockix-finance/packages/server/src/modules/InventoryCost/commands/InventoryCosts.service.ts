...
export class InventoryItemCostService {
  ...
  private getItemInventoryMeta(
    INValuationMap: Record<string, IInventoryItemCostMeta | undefined>,
    OUTValuationMap: Record<string, IInventoryItemCostMeta | undefined>,
    itemId: number,
  ) {
    const INCost = Number(get(INValuationMap, `${itemId}.cost`, 0));
    const INQuantity = Number(get(INValuationMap, `${itemId}.quantity`, 0));

    const OUTCost = Number(get(OUTValuationMap, `${itemId}.cost`, 0));
    const OUTQuantity = Number(get(OUTValuationMap, `${itemId}.quantity`, 0));
