...
export const tenantDeployments = pgTable(
  "tenant_deployments",
  {
    ...
    /** Default Finance (Bigcapital) branch ID — used by PMS finance-sync and POS multi-location reporting. */
...
export const branchLocationMappings = pgTable(
  "branch_location_mappings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
      .notNull()
    ...
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    financeBranchId: integer("finance_branch_id").notNull(),
    posLocationId: text("pos_location_id").notNull(),
    financeBranchName: text("finance_branch_name"),
  ...
  (t) => [
    index("branch_location_mappings_tenant_idx").on(t.tenantId),
    uniqueIndex("blm_org_finance_branch_unique").on(t.organizationId, t.financeBranchId),
    uniqueIndex("blm_org_pos_location_unique").on(t.organizationId, t.posLocationId),
  ],
);
...
export const apiKeys = pgTable(
  "api_keys",
  {
    ...
    /** Optional permission scope. Null = inherit owner's permissions. Non-null = restricted subset. */
